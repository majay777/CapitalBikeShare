...
st.subheader("Citywide Trip Flow")
...
view_state = pdk.ViewState(latitude=38.9, longitude=-77.03, zoom=11, pitch=45)
...
# Limit to top N stations to avoid rendering a huge matrix
TOP_N = 30
top_stations = df["start_station_name"].value_counts().head(TOP_N).index

od_matrix = pd.crosstab(
    df.loc[df["start_station_name"].isin(top_stations), "start_station_name"],
    df.loc[df["end_station_name"].isin(top_stations), "end_station_name"]
)

fig = px.imshow(
    od_matrix,
    color_continuous_scale="Blues",
    title=f"Origin-Destination Matrix (top {TOP_N} stations)"
)
st.plotly_chart(fig, use_container_width=True)
...
st.dataframe(od_filtered, use_container_width=True)
