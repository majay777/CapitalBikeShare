...
view = pdk.ViewState(latitude=38.9, longitude=-77.03, zoom=11)

st.pydeck_chart(pdk.Deck(layers=[heatmap_layer], initial_view_state=view))

# ── Animated Hourly Density ───────────────────────────────────────────────────

st.subheader("Hourly Station Density")

# Round coordinates to reduce groupby cardinality and keep the dataframe small
heat_df = (
    df
    .assign(
