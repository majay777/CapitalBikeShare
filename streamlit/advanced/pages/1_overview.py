...
member_counts.columns = ["user_type", "count"]

fig = px.pie(member_counts, values="count", names="user_type")
st.plotly_chart(fig, use_container_width=True)
...
bike_counts.columns = ["bike_type", "count"]

fig = px.pie(bike_counts, values="count", names="bike_type")
st.plotly_chart(fig, use_container_width=True)

avg_duration = (
    df.groupby("rideable_type")["duration_min"]
    .mean()
    .reset_index()
)
...
fig = px.bar(
    ...
    y="duration_min",
    color="rideable_type",
    title="Average Trip Duration by Bike Type",
    labels={"rideable_type": "Bike Type", "duration_min": "Avg Duration (min)"}
)
st.plotly_chart(fig, width='stretch')
